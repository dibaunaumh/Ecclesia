...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'Voting.decision_story'
        db.delete_column('voting_voting', 'decision_story_id')

        # Adding field 'Voting.decision'
        db.add_column('voting_voting', 'decision', self.gf('django.db.models.fields.related.OneToOneField')(blank=True, related_name='voting', unique=True, null=True, to=orm['voting.Decision']), keep_default=False)

        # Deleting field 'Decision.discussion'
        db.delete_column('voting_decision', 'discussion_id')

        # Deleting field 'Decision.voting'
        db.delete_column('voting_decision', 'voting_id')


    def backwards(self, orm):
        
        # Adding field 'Voting.decision_story'
        db.add_column('voting_voting', 'decision_story', self.gf('django.db.models.fields.related.ForeignKey')(related_name='voting', null=True, to=orm['discussions.Story'], blank=True), keep_default=False)

        # Deleting field 'Voting.decision'
        db.delete_column('voting_voting', 'decision_id')

        # We cannot add back in field 'Decision.discussion'
        raise RuntimeError(
            "Cannot reverse this migration. 'Decision.discussion' and its values cannot be restored.")

        # We cannot add back in field 'Decision.voting'
        raise RuntimeError(
            "Cannot reverse this migration. 'Decision.voting' and its values cannot be restored.")
    ...
    models = {
        ...
        'voting.voting': {
            ...
            'created_by': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['auth.User']"}),
            'decision': ('django.db.models.fields.related.OneToOneField', [], {'blank': 'True', 'related_name': "'voting'", 'unique': 'True', 'null': 'True', 'to': "orm['voting.Decision']"}),
